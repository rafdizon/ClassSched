...
class EditSchedule extends StatefulWidget {
  ...
  final int semNo;
  const EditSchedule({Key? key, required this.schedule, required this.semNo})
      : super(key: key);
...
class _EditScheduleState extends State<EditSchedule> {
  ...
  void initState() {
    ...
    _curriculumList = widget.schedule
      .where((sched) =>
          sched['cycle'] != null &&
          sched['cycle']['semester'] != null &&
          sched['cycle']['semester']['number'] == widget.semNo)
      .toList();

    for (var sched in _curriculumList) {
      int key = sched['id'] ?? sched['curriculum']['id'];
      final entry = ScheduleEntry(
        scheduleId: sched['id'],
        selectedCycle: sched['cycle']['id'],
        selectedInstructor: sched['instructor']['id'],
      );
      entry.startTimeController.text = _formatTime(sched['start_time']);
      entry.endTimeController.text = _formatTime(sched['end_time']);
      if (sched.containsKey('days')) {
        List<dynamic> days = sched['days'];
  ...
  /// Parses a time string (e.g. "8:00 AM") into minutes since midnight.
  ...
  /// Checks for scheduling conflicts between entries.
  bool _hasConflict(int key, List<Map<String, dynamic>> visibleSubjects) {
    ...
    for (var subj in visibleSubjects) {
      int otherKey = (subj['id'] != null) ? subj['id'] : subj['curriculum']['id'];
  ...
  /// Converts boolean day selections to a list of day abbreviations.
  ...
  Future _saveSchedules() async {
    ...
    final visibleSubjects = _curriculumList.where((subj) {
      int key = (subj['id'] != null) ? subj['id'] : subj['curriculum']['id'];
      return !_removedSubjectIds.contains(key);
    }).toList();

    for (var subj in visibleSubjects) {
      int key = subj['id'] ?? subj['curriculum']['id'];
    ...
    for (var key in _scheduleEntries.keys) {
      ...
      if(scheduleTimeId != null) {
        error = await adminDBManager.updateScheduleSection(
          ...
          sectionId: widget.schedule.first['section']['id'],
                                  ...
                                  children: [
                                    TableRow(
                                      children: [
                                            ...
                                            onPressed: () {
                                              showDialog(
                                                ...
                                                builder: (context) => AlertDialog(
                                                  title: Text('Remove ${subj['curriculum']['subject']['name']}?'),
                                        ...
                                        Text(
                                          subj['curriculum']['subject']['code'],
                                          style: Theme.of(context).textTheme.bodySmall,
                                        ),
                                        Text(
                                          subj['curriculum']['subject']['name'],
                                          style: Theme.of(context).textTheme.bodySmall,
                                        ),
                                        // Cycle dropdown.
                                        FutureBuilder(
          ...
          TextButton(
            onPressed: () async {
              final selectedSubject = await showDialog(
                context: context, 
                builder: (context) {
                  return AddSubjectToSchedDialog(courseId: widget.schedule.first['curriculum']['course']['id']);
                }
              );
              if (selectedSubject != null) {
                setState(() {
                  ...
                  : {
                      ...
                      "start_time": "",   // default empty string
                      "end_time": "",     // default empty string
                      "days": [],         // default empty list
                      "curriculum": {
                        "id": selectedSubject["id"],
                        "subject": selectedSubject["subject"], // FIX: use the nested subject details
                        "year_level": widget.schedule.first['section']['year_level'],
